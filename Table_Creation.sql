...
CREATE TABLE IF NOT EXISTS G1_APPLICATION (
  ...
  G1_CUST_ID INT,
  G1_APPLICATION_STATUS VARCHAR(8),
...
CREATE TABLE IF NOT EXISTS G1_PREMIUM (
  ...
  G1_PREMIIUM_PAYMENT_STATUS ENUM('PAID','NOT PAID'),
...
CREATE TABLE IF NOT EXISTS G1_SETTLEMENT (
  ...
  FOREIGN KEY (G1_CUST_ID)
  REFERENCES G1_CUSTOMER (G1_CUST_ID),
  FOREIGN KEY (G1_VEHICLE_ID)
  REFERENCES G1_VEHICLE (G1_VEHICLE_ID)
);
...
CREATE TABLE IF NOT EXISTS G1_INCIDENT (
	G1_INCIDENT_ID INT NOT NULL,
    G1_INCIDENT_TYPE VARCHAR(10),
    G1_INCIDENT_DATE DATE,
    G1_DESCRIPTION VARCHAR(100),
    PRIMARY KEY (G1_INCIDENT_ID)
);

CREATE TABLE IF NOT EXISTS G1_INCIDENT_REPORT (
	G1_REPORT_ID INT NOT NULL,
    G1_INCIDENT_ID INT,
    G1_CUST_ID INT,
    G1_INCIDENT_INSPECTOR VARCHAR(25),
    G1_INCIDENT_COST INT,
    G1_INCIDENT_TYPE VARCHAR(10),
    G1_REPORT_DESCRIPTION VARCHAR(100),
    PRIMARY KEY (G1_REPORT_ID),
    FOREIGN KEY (G1_INCIDENT_ID)
    REFERENCES G1_INCIDENT (G1_INCIDENT_ID),
    FOREIGN KEY (G1_CUST_ID)
    REFERENCES G1_CUSTOMER (G1_CUST_ID)
);

CREATE TABLE IF NOT EXISTS G1_PRODUCT (
	G1_PRODUCT_ID INT NOT NULL,
    G1_COMPANY_NAME VARCHAR(20),
    G1_PRODUCT_PRICE INT,
    G1_PRODUCT_TYPE VARCHAR(10),
    PRIMARY KEY (G1_PRODUCT_ID),
    FOREIGN KEY (G1_COMPANY_NAME)
    REFERENCES G1_COMPANY (G1_COMPANY_NAME)
);

CREATE TABLE IF NOT EXISTS G1_POLICY_RENEWABLE (
	G1_POLICY_RENEWABLE_ID INT NOT NULL,
    G1_AGREEMENT_ID INT,
    G1_DATE_RENEWAL DATE,
    G1_TYPE_RENEWAL VARCHAR(10),
    PRIMARY KEY (G1_POLICY_RENEWABLE_ID),
    FOREIGN KEY (G1_AGREEMENT_ID)
    REFERENCES G1_INSURANCE_POLICY (G1_AGREEMENT_ID)
);
